defmodule Merlin.Companies do
  ...
  defp broadcast({:ok, %Application{} = application}, event)
    ...
    Phoenix.PubSub.broadcast!(Merlin.PubSub, "new_application", {event, application})
    {:ok, application}
  end

  defp broadcast({number, nil}, event)
       when event in [:deleted_application] do
    Phoenix.PubSub.broadcast!(Merlin.PubSub, "deleted_application", {event, nil})
    {number, nil}
  end
